...
const Dashboard = () => {
  ...
  // ✅ Fetch total advance amount from accounts
  const fetchTotalAdvance = useCallback(async () => {
    try {
      ...
      if (data.success) {
        ...
        accounts.forEach(acc => {
          totalAdvance += parseFloat(acc.advance_amount || 0);
        });
  ...
  const handleRefresh = useCallback(() => {
    ...
    fetchRecentAlerts();
    fetchTotalAdvance();
  }, [fetchAllData, fetchLoanData, fetchRecentAlerts, fetchTotalAdvance]);
  ...
  const getExportData = useCallback(() => {
    ...
    return [
      ...
      {
        metric: 'Total Advance Payment',
        value: totalAdvanceAmount || 0,
        branch: getBranchDisplayName()
      }
    ];
  }, [dashboardData, loanData, totalAdvanceAmount]);
  ...
  // ✅ ALL 7 STATS CARDS with proper keys and colors
  // ✅ ORDER: Advance Pay comes before Loan Pending
  const allStats = [
    { key: 'customers', label: 'Total Customers', value: data.total_customers?.toLocaleString() || '0', icon: Users, subtitle: getBranchDisplayName(), color: '#4338ca', bg: '#e0e7ff' },
    { key: 'accounts', label: 'New Accounts', value: data.new_accounts || 0, icon: Calendar, subtitle: statsMonthLabel, color: '#b45309', bg: '#fef3c7' },
    { key: 'sales', label: 'Total Sales', value: formatCurrency(data.total_sales || 0), icon: DollarSign, subtitle: 'Lifetime revenue', color: '#b91c1c', bg: '#fee2e2' },
    { key: 'monthlySales', label: 'Monthly Sales', value: formatCurrency(data.monthly_sales || 0), icon: TrendingUp, subtitle: statsMonthLabel, color: '#2563eb', bg: '#dbeafe' },
    { key: 'recovery', label: 'Monthly Recovery', value: formatCurrency(data.monthly_recovery || 0), icon: TrendingUp, subtitle: statsMonthLabel, color: '#15803d', bg: '#dcfce7' },
    { key: 'advance', label: 'Advance Pay', value: formatCurrency(totalAdvanceAmount || 0), icon: Wallet, subtitle: 'Total advance received', color: '#8b5cf6', bg: '#faf5ff' },
    { key: 'loan', label: 'Loan Pending', value: formatCurrency(loanData.total_loans_pending || 0), icon: Landmark, subtitle: isSpecificMonth ? statsMonthLabel : 'Total loans pending', color: '#7c3aed', bg: '#ede9fe' },
  ];

  // ✅ Show hide logic for specific month
  ...
  return (
    <div className="dashboard-container">
      ...
      {/* ✅ Stats grid - 4 cards in first row, 3 in second row */}
      <div className="stats-grid-custom">
        {stats.map((stat) => (
          <div 
            key={stat.key} 
            className={`stat-card-custom stat-key-${stat.key}`}
            style={{
              borderTop: `4px solid ${stat.color}`,
              background: `linear-gradient(135deg, #ffffff 0%, ${stat.bg} 100%)`
            }}
          >
            <div className="stat-card-custom-top">
              <div className="stat-card-custom-icon" style={{ 
                background: stat.bg,
                color: stat.color
              }}>
                <stat.icon size={18} />
              </div>
              <span className="stat-card-custom-label">{stat.label}</span>
            </div>
            <span className="stat-card-custom-value" style={{ color: stat.color }}>
              {stat.value}
            </span>
            {stat.subtitle && (
              <span className="stat-card-custom-sub" style={{ 
                color: stat.color,
                background: stat.bg
              }}>
                {stat.subtitle}
