...
const EmployeePerformanceReport = () => {
  ...
  // ✅ FIXED: isAccountOverdue - due_date-based
  ...
  // ✅ FIXED: getOverdueMonthsCount - due_date-based
  ...
  // ✅ FIXED: getAccountBadgeInfo - Account Holders page logic ke mutabiq
  const getAccountBadgeInfo = (account) => {
    const list = Array.isArray(account.installments) ? account.installments : [];
    const totalInstallments = account.totalInstallments || list.length;
    const fullyPaidCount = list.filter(p => parseFloat(p.balance || 0) <= 0).length;

    // Agar koi installment nahi hai
    if (list.length === 0) {
      if (account.balance <= 0) return { key: 'clear', label: 'Clear' };
      return { key: 'unpaid', label: 'Unpaid' };
    }

    // Agar saari installments paid hain
    ...
    // Agar current month ki installment paid hai
    ...
    // Past due wali unpaid (overdue/aging)
    ...
    const overdueDays = Math.floor((today - new Date(oldestDue.due_date)) / (1000 * 60 * 60 * 24));
    ...
    return { key: 'aging', label: `Aging (${overdueMonthsApprox}m)` };
  };

  const getStatusForAccount = (account) => {
    const badge = getAccountBadgeInfo(account);
    return badge.key;
  };
  ...
  const getExportData = useCallback(() => {
    ...
    return accountsToExport.map(acc => {
      const badge = getAccountBadgeInfo(acc);
                      ...
                      <tr key={item.id} className={`${badge.key === 'overdue' || badge.key === 'aging' ? 'epr-overdue-row' : ''} ${index % 2 === 0 ? 'epr-even-row' : 'epr-odd-row'}`}>
                        <td>
                          <div className="epr-customer-info">
                            <div className="epr-customer-avatar" style={{ background: badge.key === 'clear' ? '#d1fae5' : badge.key === 'overdue' ? '#fee2e2' : badge.key === 'aging' ? '#fef3c7' : badge.key === 'paid' ? '#dbeafe' : '#e5e7eb', color: badge.key === 'clear' ? '#065f46' : badge.key === 'overdue' ? '#991b1b' : badge.key === 'aging' ? '#92400e' : badge.key === 'paid' ? '#1e40af' : '#374151' }}>
