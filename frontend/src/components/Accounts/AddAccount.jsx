...
const AddAccount = () => {
  ...
  const [audioRefs, setAudioRefs] = useState({}); // ✅ Store audio objects
  ...
  // ============================================
  // ✅ FIXED: playVoice function with proper stop
  // ============================================
  const playVoice = (index) => {
    ...
    // Agar koi aur audio play ho rahi hai toh usko stop karo
    if (playingIndex !== null) {
      const prevAudio = audioRefs[playingIndex];
      if (prevAudio) {
        prevAudio.pause();
    ...
    // Audio khatam hone par reset karo
    audio.onended = () => {
      setPlayingIndex(null);
      // Cleanup
      setAudioRefs(prev => {
    ...
    // Agar koi error aaye toh reset karo
    audio.onerror = () => {
      setPlayingIndex(null);
      setAudioRefs(prev => {
        const newRefs = { ...prev };
  ...
  // ============================================
  // ✅ FIXED: deleteVoice with proper cleanup
  // ============================================
  const deleteVoice = (index) => {
    if (window.confirm('Delete this voice file?')) {
      // Agar yehi file play ho rahi hai toh stop karo
      if (playingIndex === index) {
        const audio = audioRefs[index];
        if (audio) {
          audio.pause();
              ...
              {voiceFiles.length < MAX_VOICE_FILES && (
                <div className="voice-upload">
                  <div className="upload-area voice-upload-area" onClick={() => voiceFileRef.current?.click()} style={{ 
                    ...
                    <span className="file-hint" style={{ fontWeight: 500 }}>MP3 , WAV , M4A , OGG , WEBM(Max 20MB) — {voiceFiles.length}/{MAX_VOICE_FILES} used</span>
                  </div>
