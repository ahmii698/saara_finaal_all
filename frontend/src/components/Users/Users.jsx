...
// ============================================
// ✅ UPDATED: EditPaymentModal - With Advance Payment Support
// ============================================
const EditPaymentModal = ({
  ...
  const handleMonthChange = (e) => {
    ...
    setEditPaymentData({
      ...
      advance_amount: selected.advance_amount || 0,
      paid_amount: ''
    });
  };
      ...
      <div className="users-modal-content edit-modal" onClick={(e) => e.stopPropagation()} style={{ maxWidth: '560px' }}>
        ...
        <div className="users-modal-body">
          <div style={{
            ...
            <div>
              <span style={{ fontSize: '12px', fontWeight: 600, color: '#6b7280' }}>Advance Payment</span>
              <p style={{ fontWeight: 700, fontSize: '15px', color: '#8b5cf6', margin: 0 }}>
                {formatCurrency(editPaymentData.advance_amount || 0)}
              </p>
            </div>
          </div>
          ...
          <div className="form-group" style={{ marginBottom: '16px' }}>
            ...
            >
              {availableInstallments.map((inst) => {
                ...
                return (
                  <option key={inst.id} value={inst.id} disabled={isPaid}>
                    {inst.label}
                    {isPaid ? ' — Paid' : ' — Unpaid'}
                    {inst.advance_amount > 0 ? ` (Adv: ${formatCurrency(inst.advance_amount)})` : ''}
                  </option>
                );
              })}
            </select>
            <small style={{ display: 'block', marginTop: '6px', color: '#6b7280', fontWeight: 600 }}>
              Kisi bhi unpaid month ko select kar ke payment kar sakte hain.
            </small>
          </div>
          ...
          <div className="form-group" style={{ marginBottom: '16px' }}>
            ...
            <input
              ...
              min="0"
              max={maxPayable + (editPaymentData.advance_amount || 0)}
              autoFocus
              style={{
                width: '100%',
                padding: '10px 12px',
            ...
            />
            <small style={{ display: 'block', marginTop: '6px', color: '#6b7280', fontWeight: 600 }}>
              Max: {formatCurrency(maxPayable + (editPaymentData.advance_amount || 0))} 
              {editPaymentData.advance_amount > 0 ? ` (includes ${formatCurrency(editPaymentData.advance_amount)} advance)` : ''}
            </small>
          </div>
          ...
          <div className="form-group" style={{ marginBottom: '16px' }}>
            ...
            <textarea
              ...
              className="form-input"
              rows="3"
              style={{ resize: 'vertical', minHeight: '60px', fontFamily: 'inherit', width: '100%', padding: '10px 12px', borderRadius: '8px', border: '1px solid #d1d5db', fontWeight: 500, fontSize: '14px' }}
            />
            <small style={{ display: 'block', marginTop: '6px', color: '#6b7280', fontWeight: 600 }}>
...
const CUSTOMER_IMAGE_FIELDS = [
  ...
  'bill_image_1', 'bill_image_2', 
  'voice_consent', 'voice_consent_2', 'voice_consent_3', 'voice_consent_4',
  'voice_consent_5', 'voice_consent_6', 'voice_consent_7', 'voice_consent_8',
  'voice_consent_9', 'voice_consent_10'
];
...
const UsersManagement = () => {
  ...
  // ✅ UPDATED: fetchAccountInstallmentsList - advance_amount include karo
    ...
    try {
      ...
      if (data.success && Array.isArray(data.data)) {
        return data.data.map(inst => ({
          ...
          advance_amount: parseFloat(inst.advance_amount || 0) // ✅ ADDED
        }));
      }
      return [];
    } catch (error) {
  ...
  // ✅ FIXED: fetchClients - Ledger-based advance from account.advance_amount
    ...
    try {
      ...
      if (data.success) {
        ...
        const clientsData = accounts.map((account) => {
          ...
          // ✅ FIX: Ledger-based advance - seedha account.advance_amount use karo
          // Ab backend calculateLedgerAdvance() se advance amount aayega
          return {
            id: account.id,
            ...
            advanceAmount: parseFloat(account.advance_amount) || 0, // ✅ FIXED
            ...
            voice_consent_2: customer.voice_consent_2 || null,
            voice_consent_3: customer.voice_consent_3 || null,
            voice_consent_4: customer.voice_consent_4 || null,
            voice_consent_5: customer.voice_consent_5 || null,
            voice_consent_6: customer.voice_consent_6 || null,
            voice_consent_7: customer.voice_consent_7 || null,
            voice_consent_8: customer.voice_consent_8 || null,
            voice_consent_9: customer.voice_consent_9 || null,
            voice_consent_10: customer.voice_consent_10 || null,
  ...
  const handlePartialPaymentSubmit = async () => {
    ...
    const currentAdvance = parseFloat(editPaymentData.advance_amount) || 0;
    const totalPayable = maxPayable + currentAdvance;

    if (amount > 0 && amount > totalPayable) {
      showToaster(`Amount cannot exceed total payable of ${formatCurrency(totalPayable)}`, 'error');
      return;
    }
  ...
  const startEdit = () => {
    ...
    setEditUserData({
      ...
      phone_2: selectedUser.phone_2 || '',
      phone_3: selectedUser.phone_3 || '',
      phone_4: selectedUser.phone_4 || '',
  ...
  const handleSaveUserEdit = async () => {
    ...
    try {
      ...
      if (customerId) {
        ...
        fd.append('phone_2', editUserData.phone_2 || '');
        fd.append('phone_3', editUserData.phone_3 || '');
        fd.append('phone_4', editUserData.phone_4 || '');
  ...
  // ✅ UPDATED: getExportData - advanceAmount include karo
  const getExportData = useCallback(() => {
    return filteredData.map(client => {
      ...
      return {
        ...
        advanceAmount: client.advanceAmount || 0, // ✅ ADDED
  ...
  // ✅ UPDATED: exportColumns - Advance Payment add karo
  const exportColumns = useMemo(() => [
    ...
    { header: 'Advance Payment', key: 'advanceAmount' }, // ✅ ADDED
  ...
  // ✅ UPDATED: getClientDetailExportData - advance_amount include karo
  const getClientDetailExportData = useCallback(() => {
    ...
    return installments.map((p) => ({
      ...
      advanceAmount: selectedUser.advanceAmount || 0, // ✅ ADDED
      ...
      installmentAdvance: p ? formatCurrency(parseFloat(p.advance_amount || 0)) : 0, // ✅ ADDED
    }));
  }, [selectedUser]);

  // ✅ UPDATED: clientDetailExportColumns - Installment Advance add karo
  const clientDetailExportColumns = useMemo(() => [
    ...
    { header: 'Balance', key: 'balance' },
    { header: 'Monthly Installment', key: 'monthlyInstallment' },
    { header: 'Advance Amount', key: 'advanceAmount' }, // ✅ ADDED
    { header: 'Installments Paid', key: 'installmentsPaid' },
    { header: 'Total Installments', key: 'totalInstallments' },
    { header: 'Next Due Date', key: 'nextDueDate' },
    { header: 'Joining Date', key: 'joiningDate' },
    ...
    { header: 'Installment Advance', key: 'installmentAdvance' }, // ✅ ADDED
  ], []);
  ...
  // ✅ UPDATED: renderClientsTable - PAID COLUMN REMOVED
        ...
        <table className="users-table clients-table">
          <thead>
            <tr style={{ background: '#1E1B4B' }}>
              ...
              <th style={{ fontWeight: 800, color: '#fff', padding: '14px 16px', textAlign: 'left', borderBottom: 'none' }}>Adv Pay</th>
              ...
              data.map((client, index) => {
                ...
                return (
                  <tr key={client.id} className={getRowColorClass(client)}>
                    ...
                    <td className={client.advanceAmount > 0 ? 'advance-amount' : 'text-gray'} style={{ fontWeight: 700 }}>
                      {formatCurrency(client.advanceAmount)}
                    </td>
            ...
            <div className="users-modal-body">
                  ...
                  <div className="user-detail-item">
                    ...
                    {isEditingUser ? (
                      <>
                        ...
                        <input
                          type="text"
                          value={editUserData.phone_2 || ''}
                          onChange={(e) => setEditUserData({ ...editUserData, phone_2: e.target.value })}
                          placeholder="Phone 2 (Optional)"
                          style={{ width: '100%', padding: '6px 8px', border: '1px solid #d1d5db', borderRadius: '6px', fontWeight: 600, marginTop: '4px' }}
                        />
                        <input
                          type="text"
                          value={editUserData.phone_3 || ''}
                          onChange={(e) => setEditUserData({ ...editUserData, phone_3: e.target.value })}
                          placeholder="Phone 3 (Optional)"
                          style={{ width: '100%', padding: '6px 8px', border: '1px solid #d1d5db', borderRadius: '6px', fontWeight: 600, marginTop: '4px' }}
                        />
                        <input
                          type="text"
                          value={editUserData.phone_4 || ''}
                          onChange={(e) => setEditUserData({ ...editUserData, phone_4: e.target.value })}
                          placeholder="Phone 4 (Optional)"
                          style={{ width: '100%', padding: '6px 8px', border: '1px solid #d1d5db', borderRadius: '6px', fontWeight: 600, marginTop: '4px' }}
                        />
                      </>
                    ) : (
                      <>
                        ...
                        {selectedUser.phone_2 && (
                          <span style={{ fontSize: '13px', color: '#6b7280', display: 'block' }}>{selectedUser.phone_2}</span>
                        )}
                        {selectedUser.phone_3 && (
                          <span style={{ fontSize: '13px', color: '#6b7280', display: 'block' }}>{selectedUser.phone_3}</span>
                        )}
                        {selectedUser.phone_4 && (
                          <span style={{ fontSize: '13px', color: '#6b7280', display: 'block' }}>{selectedUser.phone_4}</span>
                        )}
                      </>
                    )}
                  </div>
              ...
              <div className="detail-section">
                ...
                <div className="account-summary-grid" style={{
                  ...
                  <div className="summary-item advance" style={{ background: '#ede9fe', padding: '12px', borderRadius: '8px', border: '1px solid #8b5cf6' }}>
                    <span style={{ fontWeight: 700, fontSize: '12px', color: '#5b21b6' }}>Advance Payment</span>
                    <strong style={{ fontWeight: 700, fontSize: '15px', color: '#5b21b6', display: 'block' }}>
                ...
                <div style={{ marginBottom: '20px' }}>
                  ...
                  {isEditingUser ? (
                    ...
                    <>
                      ...
                      {selectedUser.voice_consent_2 && (
                        <audio controls preload="none" style={{ width: '100%', marginBottom: '8px' }}>
                          <source src={getFileUrl(selectedUser.voice_consent_2)} />
                        </audio>
                      )}
                      {selectedUser.voice_consent_3 && (
                        <audio controls preload="none" style={{ width: '100%', marginBottom: '8px' }}>
                          <source src={getFileUrl(selectedUser.voice_consent_3)} />
                        </audio>
                      )}
                      {selectedUser.voice_consent_4 && (
                        <audio controls preload="none" style={{ width: '100%', marginBottom: '8px' }}>
                          <source src={getFileUrl(selectedUser.voice_consent_4)} />
                        </audio>
                      )}
                      {selectedUser.voice_consent_5 && (
                        <audio controls preload="none" style={{ width: '100%', marginBottom: '8px' }}>
                          <source src={getFileUrl(selectedUser.voice_consent_5)} />
                        </audio>
                      )}
                      {selectedUser.voice_consent_6 && (
                        <audio controls preload="none" style={{ width: '100%', marginBottom: '8px' }}>
                          <source src={getFileUrl(selectedUser.voice_consent_6)} />
                        </audio>
                      )}
                      {selectedUser.voice_consent_7 && (
                        <audio controls preload="none" style={{ width: '100%', marginBottom: '8px' }}>
                          <source src={getFileUrl(selectedUser.voice_consent_7)} />
                        </audio>
                      )}
                      {selectedUser.voice_consent_8 && (
                        <audio controls preload="none" style={{ width: '100%', marginBottom: '8px' }}>
                          <source src={getFileUrl(selectedUser.voice_consent_8)} />
                        </audio>
                      )}
                      {selectedUser.voice_consent_9 && (
                        <audio controls preload="none" style={{ width: '100%', marginBottom: '8px' }}>
                          <source src={getFileUrl(selectedUser.voice_consent_9)} />
                        </audio>
                      )}
                      {selectedUser.voice_consent_10 && (
                        <audio controls preload="none" style={{ width: '100%', marginBottom: '8px' }}>
                          <source src={getFileUrl(selectedUser.voice_consent_10)} />
                        </audio>
                      )}
                      {!selectedUser.voice_consent && !selectedUser.voice_consent_2 && !selectedUser.voice_consent_3 && !selectedUser.voice_consent_4 && !selectedUser.voice_consent_5 && !selectedUser.voice_consent_6 && !selectedUser.voice_consent_7 && !selectedUser.voice_consent_8 && !selectedUser.voice_consent_9 && !selectedUser.voice_consent_10 && (
                        <p style={{ color: '#6b7280', fontSize: '13px', margin: 0 }}>No voice consent uploaded</p>
                      )}
                    </>
                  )}
                </div>
                    ...
                    <table className="history-table">
                      <thead>
                        <tr>
                          ...
                          <th style={{ fontWeight: 700 }}>Advance</th>
                        </tr>
                      </thead>
                      <tbody>
                        {selectedUser.installments.slice(0, 10).map((p, idx) => (
                          <tr key={p.id} className={p.balance <= 0 ? 'history-paid' : ''}>
                            ...
                            <td>{formatCurrency(p.advance_amount || 0)}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
