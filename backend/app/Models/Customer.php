...
class Customer extends Model
{
    ...
    protected $fillable = [
        ...
        // ✅ UPDATED: ab 4 ki bajaye 10 voice files support hongi
    ...
    // ✅ NEW: voice_consent_5 se voice_consent_10 tak accessors
    public function getVoiceConsent5UrlAttribute()
    {
        if ($this->voice_consent_5 && Storage::disk('public')->exists($this->voice_consent_5)) {
            return asset('storage/' . $this->voice_consent_5);
        }
        return null;
    }

    public function getVoiceConsent6UrlAttribute()
    {
        if ($this->voice_consent_6 && Storage::disk('public')->exists($this->voice_consent_6)) {
            return asset('storage/' . $this->voice_consent_6);
        }
        return null;
    }

    public function getVoiceConsent7UrlAttribute()
    {
        if ($this->voice_consent_7 && Storage::disk('public')->exists($this->voice_consent_7)) {
            return asset('storage/' . $this->voice_consent_7);
        }
        return null;
    }

    public function getVoiceConsent8UrlAttribute()
    {
        if ($this->voice_consent_8 && Storage::disk('public')->exists($this->voice_consent_8)) {
            return asset('storage/' . $this->voice_consent_8);
        }
        return null;
    }

    public function getVoiceConsent9UrlAttribute()
    {
        if ($this->voice_consent_9 && Storage::disk('public')->exists($this->voice_consent_9)) {
            return asset('storage/' . $this->voice_consent_9);
        }
        return null;
    }

    public function getVoiceConsent10UrlAttribute()
    {
        if ($this->voice_consent_10 && Storage::disk('public')->exists($this->voice_consent_10)) {
            return asset('storage/' . $this->voice_consent_10);
        }
        return null;
    }
