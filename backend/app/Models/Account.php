...
class Account extends Model
{
    ...
    // ============================================
    // ✅ NEW: LEDGER-BASED ADVANCE CALCULATION
    // Advance = ab tak jitna total paid hua − jitna due_date <= aaj tak
    // technically "due" ho chuka hai. Isse pehle-se-pay-kiya-hua paisa
    // hamesha advance dikhega, jab tak uski apni due_date na aa jaye.
    // ============================================
    public function calculateLedgerAdvance()
    {
        $today = now()->toDateString();

        $totalPaid = $this->installments()->sum('paid_amount');

        $totalDueSoFar = $this->installments()
            ->whereNotNull('due_date')
            ->whereDate('due_date', '<=', $today)
            ->sum('due_amount');

        return round(max(0, $totalPaid - $totalDueSoFar), 2);
    }
